...
function run() {
  ...
  let state = { selectedPreset: 'blood-20', intervalMinutes: 20, customIntervalMinutes: '', lastCompletedAtEpoch: null, nextDueAtEpoch: null, doseLog: [] };
  ...
  assert.strictEqual(state.nextDueAtEpoch, 100000 + 20 * 60000, 'nextDueAtEpoch should be completion plus interval minutes');
  ...
  assert.strictEqual(state.nextDueAtEpoch, 100000 + 20 * 60000, 'Undo last should restore previous next due from interval');
  ...
  const activeState = { ...state, lastCompletedAtEpoch: 100000, nextDueAtEpoch: 100000 + 20 * 60000 };
  ...
  assert(mainJs.includes("root.classList.toggle('hidden', timeLiveMode !== 'live')"), 'reminder card should stay subordinate by showing only in Live mode');
