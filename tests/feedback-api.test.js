...
async function run() {
  ...
  try {
    ...
    assert(mainJs.includes("el(isTransplantMode ? 'time-transplant-content' : 'time-case-summary')"), 'Transplant feedback should anchor outside the calculator subtree that rerenders');
    assert(mainJs.includes('Anchor feedback after the stable mode container so an active card survives.'), 'Transplant rerenders should explicitly preserve an active feedback card');
