...
function renderBmi25ReferenceFlowList(bmi25ReferenceBsa) {
  const bmi25FlowList = el('bsa-bmi25-flow-list');
  if (!bmi25FlowList) return;

  bmi25FlowList.innerHTML = '';
  if (!(bmi25ReferenceBsa > 0)) return;

  for (let ciTenths = 10; ciTenths <= 30; ciTenths += 2) {
    const ci = ciTenths / 10;
    const flow = ci * bmi25ReferenceBsa;
    const row = document.createElement('div');
    const isCi24 = Math.abs(ci - 2.4) < 0.05;
    row.className = `grid grid-cols-[1fr_auto] items-center py-1.5 px-2 text-sm border-b border-blue-100 dark:border-blue-900/60 last:border-0 gap-3 ${isCi24 ? 'bg-blue-100/70 dark:bg-blue-800/30' : ''}`;
...
function updateStandaloneBsa() {
  ...
  const bmi25FlowCard = el('bsa-bmi25-flow-card');
  const bmi25ReferenceBsaEl = el('bsa-bmi25-reference-bsa');
  const bmi25ReferenceWeightEl = el('bsa-bmi25-reference-weight');
  ...
  if (bmiDisplay) {
    if (h > 0 && w > 0) {
      ...
      if (bmi25FlowCard) {
        ...
        bmi25FlowCard.classList.toggle('hidden', !isObese);
        if (isObese) {
          const bmi25ReferenceWeightKg = 25 * Math.pow(heightMeters, 2);
          const bmi25ReferenceBsa = computeBSA(h, bmi25ReferenceWeightKg, method);
          if (bmi25ReferenceBsaEl) bmi25ReferenceBsaEl.textContent = `${bmi25ReferenceBsa.toFixed(2)} m²`;
          if (bmi25ReferenceWeightEl) bmi25ReferenceWeightEl.textContent = `${bmi25ReferenceWeightKg.toFixed(1)} kg`;
          renderBmi25ReferenceFlowList(bmi25ReferenceBsa);

        } else {
          if (bmi25ReferenceBsaEl) bmi25ReferenceBsaEl.textContent = '—';
          if (bmi25ReferenceWeightEl) bmi25ReferenceWeightEl.textContent = '—';
        }
      }
    } else {
      ...
      if (bmi25FlowCard) bmi25FlowCard.classList.add('hidden');
      if (bmi25ReferenceBsaEl) bmi25ReferenceBsaEl.textContent = '—';
      if (bmi25ReferenceWeightEl) bmi25ReferenceWeightEl.textContent = '—';
    }
  }
...
async function loadCannulaPressureDropData() {
  ...
  cannulaPressureDropDataPromise = fetch(CANNULA_PRESSURE_DROP_DATA_URL, { cache: 'default' })
    ...
    .then(payload => {
      ...
      return payload.items.map(normalizePressureDropEntry);
    })
    .catch(error => {
...
function normalizePressureDropEntry(entry) {
  const rawSeries = Array.isArray(entry?.pressureSeries) && entry.pressureSeries.length
    ? entry.pressureSeries
    : [{ id: 'pressure-drop', label: 'Pressure drop', semanticType: 'pressure-drop', lineStyle: 'solid', points: entry?.points || [] }];
  const pressureSeries = rawSeries.map(series => normalizePressureDropSeries(series, `${entry?.manufacturer || 'Cannula'} ${series.label || 'series'}`));
  return { ...entry, points: pressureSeries[0].points, pressureSeries };
}

function getPressureDropSeries(entry) {
  if (Array.isArray(entry?.pressureSeries) && entry.pressureSeries.length) return entry.pressureSeries;
  return [{ id: 'pressure-drop', label: 'Pressure drop', semanticType: 'pressure-drop', lineStyle: 'solid', points: entry?.points || [] }];
}
...
function drawPressureDropSeriesChart(svgNode, pressureSeries, targetFlow, estimates = [], options = {}) {
  ...
  const rawMaxDrop = Math.max(...allPoints.map(point => point.pressureDrop));
  const crossesZero = rawMinDrop < 0 && rawMaxDrop > 0;
  const minDrop = rawMinDrop < 0 ? rawMinDrop : 0;
  const maxDrop = rawMaxDrop > 0 ? rawMaxDrop : 0;
  const pressureRange = Math.max(maxDrop - minDrop, 1);
  const scaleX = flow => padding.left + ((flow - minFlow) / Math.max(maxFlow - minFlow, 0.0001)) * (width - padding.left - padding.right);
  const scaleY = drop => height - padding.bottom - ((drop - minDrop) / pressureRange) * (height - padding.top - padding.bottom);
  ...
  const yTicks = buildPressureDropAxisTicks(minDrop, maxDrop, 5);
  const xGridlines = xTicks.map(flow => `<line x1="${scaleX(flow).toFixed(1)}" y1="${padding.top}" x2="${scaleX(flow).toFixed(1)}" y2="${plotBottom}" stroke="currentColor" stroke-opacity="0.10" stroke-width="0.75" />`).join('');
  ...
  const showRawPoints = options.showRawPoints === true;
  const traces = renderedSeries.map((item, index) => {
    ...
    const rawPointMarkers = showRawPoints
      ? item.points.map(point => `<circle data-raw-pressure-point="true" cx="${scaleX(point.flow).toFixed(1)}" cy="${scaleY(point.pressureDrop).toFixed(1)}" r="2" fill="white" stroke="${color}" stroke-width="1.2"><title>${displayLabel}; Flow: ${point.flow.toFixed(2)} L/min; Signed pressure: ${formatSignedPressureDrop(point.pressureDrop)} mmHg</title></circle>`).join('')
      : '';
    return `<g aria-label="${displayLabel} pressure series"><path d="${path}" fill="none" stroke="${color}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"${dash} />${rawPointMarkers}</g>`;
  }).join('');
...
function drawPressureDropChart(svgNode, points, targetFlow, estimatedPressureDrop, options = {}) {
  drawPressureDropSeriesChart(svgNode, [{ id: 'pressure-drop', label: 'Pressure drop', lineStyle: 'solid', points }], targetFlow, [{ value: estimatedPressureDrop }], options);
}
...
function createHctErrorResult(message, fields = {}) {
  return {
    validationMessage: message,
    invalidInput: true,
    invalidCurrentVolume: false,
    invalidFinalVolume: false,
    ...fields
  };
}
...
function firstValidationMessage(validations) {
...
function calculatePreCpbHct({ patientType, ebvCoef: ebvCoefValue, weightKg, preCpbHct, primeVolumeMl, additionalCrystalloidMl = 0, ultrafiltrationRemovedMl = 0, rbcUnits = 0, rbcVolumePerUnitMl = 300, rbcUnitHct = 60 }) {
  ...
  if (validationMessage) {
    return createHctErrorResult(validationMessage, {
      ebvMl: null,
      patientRbcMl: null,
      transfusedRbcVolumeMl: null,
      transfusedRbcCellVolumeMl: null,
      totalVolumeMl: null,
      finalRbcVolumeMl: null,
      resultHctPercent: null
    });
  }
  ...
  if (!(totalVolumeMl > 0)) {
    return createHctErrorResult('Final volume must be greater than 0. Check prime volume, RBC prime, and removals.', {
      ebvMl,
      patientRbcMl,
      transfusedRbcVolumeMl,
      transfusedRbcCellVolumeMl,
      totalVolumeMl,
      finalRbcVolumeMl,
      resultHctPercent: null,
      invalidInput: false,
      invalidFinalVolume: true
    });
  }

  const resultHctPercent = (finalRbcVolumeMl / totalVolumeMl) * 100;
  return { ebvMl, patientRbcMl, transfusedRbcVolumeMl, transfusedRbcCellVolumeMl, totalVolumeMl, finalRbcVolumeMl, resultHctPercent, validationMessage: '', invalidInput: false, invalidCurrentVolume: false, invalidFinalVolume: false };
}
...
function computeOnPumpHctAdjustment({ patientType, weightKg, ebvCoefValue, primeVolume, netIoChange = 0, currentHct, addedCrystalloid = 0, rbcUnits = 0, rbcUnitVol = 300, rbcUnitHct = 60, ultrafiltrationRemoved = 0 }) {
  ...
  const ebv = weightKg * safeEbvCoef;
  ...
  const volumeValidationMessage = invalidCurrentVolume
  ...
  if (invalidCurrentVolume) {
    return { ebv, baseCpbVolume, currentTotalVolume, currentRbcVolume: null, addedRbcVolume, finalTotalVolume, predictedHct: null, hctChange: null, invalidInput: false, invalidCurrentVolume, invalidFinalVolume, validationMessage: volumeValidationMessage };
  }
  ...
  if (invalidFinalVolume) {
    return { ebv, baseCpbVolume, currentTotalVolume, currentRbcVolume, addedRbcVolume, finalTotalVolume, predictedHct: null, hctChange: null, invalidInput: false, invalidCurrentVolume, invalidFinalVolume, validationMessage: volumeValidationMessage };
  }
  ...
  const hctChange = predictedHct - currentHct;
  return { ebv, baseCpbVolume, currentTotalVolume, currentRbcVolume, addedRbcVolume, finalTotalVolume, predictedHct, hctChange, invalidInput: false, invalidCurrentVolume, invalidFinalVolume, validationMessage: '' };
}
...
function clearTargetHctOutputs(message = 'Enter a target Hct to compare adjustment scenarios.') {
  ...
  if (dilutionCardEl) {
    dilutionCardEl.classList.add('hidden');
    dilutionCardEl.innerHTML = '';
  }
  ['target_rbc_only', 'target_rbc_neutral', 'target_hfuf_only'].forEach((id) => {
    const node = el(id);
...
function clearPredictedHctPrimaryOutputs(message = '') {
  setText('ebv', '—');
  setText('total_vol', '—');
  setText('pred_hct', '—');
  if (message) clearTargetHctOutputs(message);
}

function formatFiniteMl(value) {
  return Number.isFinite(value) ? value.toFixed(0) : '—';
}

function formatFiniteHct(value) {
...
function updateHct() {
  ...
  if (isOnPumpMode) {
    ...
    const hasOnPumpValidation = !!r.validationMessage;
    setText('ebv', hasOnPumpValidation ? '—' : formatFiniteMl(r.currentTotalVolume));
    setText('total_vol', hasOnPumpValidation ? '—' : formatFiniteMl(r.finalTotalVolume));
    setText('pred_hct', hasOnPumpValidation ? '—' : formatFiniteHct(r.predictedHct));
    setText('current_rbc_vol', Number.isFinite(r.currentRbcVolume) ? `${r.currentRbcVolume.toFixed(0)} mL` : '—');
    setText('added_rbc_vol', Number.isFinite(r.addedRbcVolume) ? `${r.addedRbcVolume.toFixed(0)} mL` : '—');
    setText('onpump_ebv', Number.isFinite(r.ebv) ? `${r.ebv.toFixed(0)} mL` : '—');
    setText('onpump_estimated_volume', Number.isFinite(r.currentTotalVolume) ? `${r.currentTotalVolume.toFixed(0)} mL` : '—');
    setText('onpump_ebv_auto', Number.isFinite(r.ebv) ? `${r.ebv.toFixed(0)} mL` : '—');
    setText('onpump_base_cpb_volume', Number.isFinite(r.baseCpbVolume) ? `${r.baseCpbVolume.toFixed(0)} mL` : '—');
    setText('onpump_estimated_auto', Number.isFinite(r.currentTotalVolume) ? `${r.currentTotalVolume.toFixed(0)} mL` : '—');
    setText('onpump_current_rbc_summary', Number.isFinite(r.currentRbcVolume) ? `${r.currentRbcVolume.toFixed(0)} mL` : '—');
    const resultMessageEl = el('onpump_result_message');
    const resultValuesEl = el('onpump_result_values');
    if (resultMessageEl) {
      resultMessageEl.textContent = r.validationMessage || '';
      resultMessageEl.classList.toggle('hidden', !r.validationMessage);
    }
    if (resultValuesEl) resultValuesEl.classList.toggle('hidden', !!r.validationMessage);
    setText('current_hct_result', r.validationMessage ? '—' : formatFiniteHct(num('current_hct')));
    setText('pred_hct_result', r.predictedHct === null ? '—' : formatFiniteHct(r.predictedHct));
    setText('hct_change', r.hctChange === null ? '' : `(${r.hctChange >= 0 ? '+' : ''}${r.hctChange.toFixed(1)} pt)`);
    setText('current_volume_result', r.validationMessage ? '—' : `${formatTargetVolume(r.currentTotalVolume)} mL`);
    setText('final_volume_result', r.validationMessage ? '—' : `${formatTargetVolume(r.finalTotalVolume)} mL`);
  ...
  if (r.validationMessage) {
    clearPredictedHctPrimaryOutputs(r.validationMessage);
    if (modeHelpEl) modeHelpEl.textContent = r.validationMessage;
    return;
  }
  if (modeHelpEl) modeHelpEl.textContent = 'Estimate dilutional hematocrit at CPB initiation.';
  setText('ebv', formatFiniteMl(r.ebv));
  setText('total_vol', formatFiniteMl(r.totalVol));
  setText('pred_hct', formatFiniteHct(r.hct));
}
...
function renderTargetScenario(elementId, scenario, { mainAction, secondaryAction, footer }) {
  ...
  if (!node) return;
  if (!scenario) {
    node.innerHTML = '<p class="onpump-result-summary__primary">Not applicable</p>';
  ...
  node.innerHTML = `<p class="onpump-result-summary__primary">${mainAction(scenario)}</p>`;
  if (secondaryNode) {
  ...
  if (dilutionCardEl) {
    ...
    if (result.noAdjustment) {
      ...
      dilutionCardEl.innerHTML = [
        ...
        `<p class="onpump-result-summary__secondary mt-0.5">Final ${formatTargetVolume(result.dilution.finalVolume)} mL · Hct ${formatTargetHct(targetHct)}%</p></div>`,
        `<p class="onpump-result-summary__primary mt-2 sm:mt-0 sm:text-right">Add crystalloid/colloid ${formatTargetVolume(result.dilution.crystalloidToAdd)} mL</p>`,
        '</div>'
      ].join('');
    }
  }
...
function getSummaryDurationMinutes(rowId, startMin, endMin, startValue, endValue) {
  ...
  const stoppedLiveStateMatchesInputs = Boolean(
    ...
    state.endAtEpoch &&
    !state.running &&
    startValue === formatEpochToHHMM(state.startAtEpoch) &&
    endValue === formatEpochToHHMM(state.endAtEpoch)
  );
  if (stoppedLiveStateMatchesInputs) {
    // Stopped Live rows display charting minutes from epoch elapsed time, so the copied
    // summary uses the same floored minute basis instead of recalculating from HH:MM.
    return Math.floor(Math.max(0, state.endAtEpoch - state.startAtEpoch) / 60000);
  }
...
function setTimeLiveMode(nextMode) {
  timeLiveMode = ['record', 'live', 'transplant'].includes(nextMode) ? nextMode : 'record';
  const recordBtn = document.getElementById('time-mode-record');
  const liveBtn = document.getElementById('time-mode-live');
  const transplantBtn = document.getElementById('time-mode-transplant');
  const notice = document.getElementById('time-live-notice');
  [recordBtn, liveBtn, transplantBtn].forEach(btn => {
    if (!btn) return;
    btn.setAttribute('aria-pressed', 'false');
    btn.classList.remove('bg-accent-600', 'text-white', 'shadow-sm');
    btn.classList.add('text-slate-600', 'dark:text-slate-200');
  });
  const activeBtn = timeLiveMode === 'live' ? liveBtn : (timeLiveMode === 'transplant' ? transplantBtn : recordBtn);
  if (activeBtn) {
    activeBtn.setAttribute('aria-pressed', 'true');
    activeBtn.classList.add('bg-accent-600', 'text-white', 'shadow-sm');
    activeBtn.classList.remove('text-slate-600', 'dark:text-slate-200');
  }
  if (notice) notice.classList.toggle('hidden', timeLiveMode !== 'live');
  document.getElementById('time-standard-content')?.classList.toggle('hidden', timeLiveMode === 'transplant');
  document.getElementById('time-transplant-content')?.classList.toggle('hidden', timeLiveMode !== 'transplant');
  renderCardioplegiaReminder();
  renderCardioplegiaShortcut();
  getOrderedTimeRows().forEach(row => {
    updateTimeRow(row.id);
...
function initTimeCalculator() {
  ...
  const transplantModeBtn = document.getElementById('time-mode-transplant');
  ...
  if (transplantModeBtn) transplantModeBtn.addEventListener('click', () => setTimeLiveMode('transplant'));
...
function renderTransplantSummary() {
  ...
  return `<section class="relative rounded-2xl border border-slate-200 dark:border-primary-700 bg-white/80 dark:bg-primary-900/50 p-4 space-y-3">
    ...
    <button id="transplant-summary-copy" type="button" class="absolute right-3 top-3 inline-flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 dark:border-primary-700 bg-white dark:bg-primary-800 text-slate-600 dark:text-slate-200 shadow-sm hover:border-accent-400 hover:text-accent-700 dark:hover:text-accent-300 focus:ring-2 focus:ring-accent-500 focus:ring-offset-2 dark:focus:ring-offset-primary-900" aria-label="Copy transplant case summary" title="Copy summary"><svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7.5V6A2.5 2.5 0 0110.5 3.5H18A2.5 2.5 0 0120.5 6v7.5A2.5 2.5 0 0118 16h-1.5M6 8h7.5A2.5 2.5 0 0116 10.5V18a2.5 2.5 0 01-2.5 2.5H6A2.5 2.5 0 013.5 18v-7.5A2.5 2.5 0 016 8z" /></svg></button>
    ...
    <span id="transplant-summary-status" class="block min-h-4 text-[11px] font-semibold text-slate-500 dark:text-slate-400" aria-live="polite"></span>
  </section>`;
}
...
function getTransplantPath(path) {
  return path.split('.').reduce((value, key) => value?.[key], transplantState) || '';
}

function setTransplantPath(path, value) {
  const keys = path.split('.');
  const finalKey = keys.pop();
  const target = keys.reduce((object, key) => object[key], transplantState);
...
function renderTransplantCalculator() {
  ...
  if (!root) return;
  root.innerHTML = transplantState.activeType === 'lung' ? renderLungTransplant() : renderHeartTransplant();
  document.querySelectorAll('.transplant-type-btn').forEach(button => {
    const active = button.id === `transplant-type-${transplantState.activeType}`;
...
function createPressureDropCandidateList(candidates, onSelect) {
  ...
  candidates.slice(0, 12).forEach(entry => {
    ...
    button.innerHTML = `<p class="text-sm font-semibold text-primary-900 dark:text-white">${entry.manufacturer} · ${entry.model}</p><p class="mt-1 text-xs text-slate-500 dark:text-slate-400">${getPressureDropGroupLabel(entry.category)} · ${entry.size || 'Unknown size'}${entry.connectionSite ? ` · ${entry.connectionSite}` : ''} · ${getPressureDropFlowRange(entry)}${getPressureDropSeries(entry).length > 1 ? ' · Dual lumen · Infusion + Drainage' : ''}</p>`;
...
function createPressureDropEstimateCard(entry, flowInputValue, flowValue, interpolationResults, onFlowInput) {
  ...
  const series = getPressureDropSeries(entry);
  const hasOutOfRange = interpolationResults.some(result => result.state === 'out_of_range');
  card.className = `rounded-xl border ${hasOutOfRange ? 'border-amber-300 dark:border-amber-500/50 bg-amber-50 dark:bg-amber-500/10' : 'border-accent-500/25 bg-accent-500/10 dark:bg-accent-500/15'} p-4 space-y-3`;
  const title = document.createElement('p');
  title.className = 'text-xs uppercase tracking-wider text-accent-700 dark:text-accent-300';
  title.textContent = series.length > 1 ? 'Independent lumen pressures' : 'Estimated pressure drop';
  ...
  const inputLabel = document.createElement('span'); inputLabel.className = 'text-xs tracking-wider text-slate-500 dark:text-slate-400'; inputLabel.textContent = 'Flow rate (L/min)';
...
// Single-series rendering remains: drawPressureDropChart(svg, entry.points, hasEstimate ? flowValue : NaN, hasEstimate ? interpolationResult.value : NaN, { curveMode: 'linear' });
function createPressureDropRawPointsToggle(checked, onChange, id) {
  const label = document.createElement('label');
  label.className = 'inline-flex w-fit cursor-pointer items-center gap-2 text-xs text-slate-600 dark:text-slate-300';
  const input = document.createElement('input');
  input.id = id;
  input.type = 'checkbox';
  input.checked = checked;
  input.className = 'h-4 w-4 rounded border-slate-300 text-accent-600 focus:ring-accent-500 dark:border-primary-600 dark:bg-primary-800';
  input.addEventListener('change', () => onChange(input.checked));
  const text = document.createElement('span');
  text.textContent = 'Show raw digitized points';
...
function createPressureDropSelectedSummary(entry) {
  ...
  [
    entry.outerDiameterMm ? ['Outer diameter', `${entry.outerDiameterMm} mm`] : null,
    entry.insertableLength ? ['Insertable length', entry.insertableLength] : null,
    ...
    entry.sapCode ? ['SAP code', entry.sapCode] : null,
    entry.testMedium ? ['Test medium', entry.testMedium] : null,
    dataStatusText && dataStatusText !== '—' ? ['Data status', dataStatusText] : null,
    getPressureDropSeries(entry).length > 1 ? ['Flow ranges', getPressureDropSeries(entry).map(series => `${series.label}: ${getPressureDropRangeText(getValidPressureDropPoints(series.points), '')}`).join('; ')] : (flowRangeText && flowRangeText !== '—' ? ['Flow range', flowRangeText] : null)
...
function createPressureDropLookupResult(entry, flowInputValue, flowValue, onFlowInput, showRawPoints, onRawPointsChange) {
  const interpolationResults = getPressureDropSeries(entry).map(series => interpolatePressureDrop(series.points, flowValue));
  const wrap = document.createElement('div'); wrap.className = 'space-y-4';
  wrap.appendChild(createPressureDropEstimateCard(entry, flowInputValue, flowValue, interpolationResults, onFlowInput));
  wrap.appendChild(createPressureDropChartPanel(entry, flowValue, interpolationResults, showRawPoints, onRawPointsChange));
  wrap.appendChild(createPressureDropSelectedSummary(entry)); return wrap;
}
...
// Legacy single-series comparisons use interpolatePressureDrop(entry.points, flowValue) through the normalized first series.
function getPressureDropComparisonResult(entry, flowValue) {
  ...
  const isHighPressure = validResults.some(item => shouldApplyPressureDropHighWarning(entry) && item.interpolationResult.value > 100);
  return {
    seriesResults,
...
async function initCannulaPressureDropPage() {
  ...
  try {
    ...
    const render = ({ focusResultFlow = false } = {}) => {
      ...
      results.appendChild(createPressureDropLookupResult(
        selectedEntry,
        flowInputValue,
        flowValue,
        value => syncFlowInput(value, { focusResultFlow: true }),
